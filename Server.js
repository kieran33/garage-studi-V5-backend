...
app.post("/ajout-voitures-vues", (req, res) => {
  ...
  VoitureModel.insertMany({ marque: marqueVoiture }, { nombreVues: 0 })
    .then((voitures) => res.json(voitures))
    .catch((voitures) => res.json(voitures));
});
...
db.connect((err) => {
  ...
  const createServicesTable = `
        CREATE TABLE IF NOT EXISTS voiture_services (
            id INT AUTO_INCREMENT PRIMARY KEY,
  ...
  db.query(createServicesTable, (err) => {
    if (err) throw err;
    console.log("Table 'voiture_services' prête");
  });
...
app.post("/create-employe", (req, res) => {
  ...
  bcrypt.hash(password, saltRounds, function (err, hash) {
    ...
    const query =
      "INSERT INTO employes (email, password, role) VALUES (?, ?, ?)";
...
app.get("/services", (req, res) => {
  const request = "SELECT * FROM voiture_services";
  db.query(request, (error, result) => {
    res.send(result);
  });
});
...
app.delete("/services/remove/:name", (req, res) => {
  ...
  const request = "DELETE FROM voiture_services WHERE name = ?";
...
app.post("/login", (req, res) => {
  ...
  db.query(
    ...
    async (err, results) => {
      ...
      if (results.length === 0) {
        //On rentre ici si l'email est incorrect
      ...
      if (user.password.includes("admin")) {
        try {
          ...
          if (isMatched) {
            ...
            res
              .status(401)
              .json({ success: false, message: "Connexion échoué" });
          }
        } catch (err) {
          console.log(err);
          res
...
app.post(
  ...
  (req, res) => {
    ...
    db.query(
      "INSERT INTO voiture_services (name, content, image) VALUES (?, ?, ?)",
...
app.post(
  ...
  (req, res) => {
    ...
    db.query(
      "INSERT INTO voitures (brand, km, price, yearsCirculation, image) VALUES (?, ?, ?, ?, ?)",
      [brand, km, price, yearsCirculation, imageName],
      (error, result) => {
        if (error) {
          console.log(error);
...
app.post("/add-avis-non-verif", (req, res) => {
  ...
  db.query(
    "INSERT INTO avisnonverif (name, message, note) VALUE (?, ?, ?)",
    [name, message, note],
    (error, result) => {
      if (error) {
        console.log(error);
...
app.post("/add-avis-verif", verifyToken, (req, res) => {
  ...
  db.query(
    "INSERT INTO avisverif (name, message, note) VALUE (?, ?, ?)",
    [name, message, note],
    (error, result) => {
      if (error) {
        console.log(error);
